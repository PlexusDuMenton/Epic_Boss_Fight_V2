	...
	if (selected_tab == "infusion"){
		if (secondary_slot != -1){ 
			if ($("#stats_gain_stats_damage") != null) {
				...
				$("#stats_gain_stats_range").text = $.Localize("#stats_range") + inventory[secondary_slot].range
	...
	if (selected_tab == "transmutation"){
		if (secondary_slot != -1){ 
			if ($("#Xp_gain") != null){
				...
				if (typeof inventory[secondary_slot].range != "undefined"){range = inventory[secondary_slot].range}
				if (typeof inventory[secondary_slot].upgrade_range != "undefined"){range = inventory[secondary_slot].upgrade_range + range}
				if (typeof inventory[secondary_slot].bonus_range != "undefined"){range = inventory[secondary_slot].bonus_range + range}
				...
				$.Msg((2+ damage * 2) ," ", ( 2 + m_damage * 2) ," ", (2 + range * 0.2) ," ", ( 2 + attack_speed) ," ", (2 + loh) ," ", (2 +Math.pow(ls,1.5)))
				$("#Xp_gain").text = $.Localize("#Xp_gain") + ( Math.ceil(inventory[secondary_slot].XP/3 + ( Math.log10( 2 +inventory[secondary_slot].XP/2) + 5*Math.pow(inventory[secondary_slot].level,1.2)) * ( 1 + (0.1*( 0.1 + Math.log10( 2 + damage * 2) + Math.log10( 2 + m_damage * 2) + Math.log10(2 + range * 0.2) + Math.log10( 2 + attack_speed) + Math.log10(2 + loh) + Math.log10(2 +Math.pow(ls,1.5)) * 10)))))
				if (main_slot != -1){
					$("#button_text").SetHasClass( "unselected_button_label", true )
				}
			}
	...
	if (selected_tab == "crystalisation"){
		if (main_slot != -1){
			if ($("#new_soul_damage") != null){
				...
				if (typeof inventory[main_slot].range == "undefined"){inventory[main_slot].range = 0}
				if (typeof inventory[main_slot].upgrade_range == "undefined"){inventory[main_slot].upgrade_range = 0}
				if (typeof inventory[main_slot].bonus_range == "undefined"){inventory[main_slot].bonus_range = 0}
				...
				$("#new_soul_range").text = $.Localize("#new_soul_range") + Math.ceil((inventory[main_slot].range + inventory[main_slot].upgrade_range + inventory[main_slot].bonus_range) * 0.25)
	...
	if (selected_tab == "evolution"){
		if (main_slot != -1){
			...
			function create_button(number,key,evolu){
				...
				Button.style.position = "100px "+(200+pos_y_button)+"px 1px";
...
function display_evolution(way,name,evolu_need,originel_item){
	...
	function display_req(cat,stat_name){
			...
			text.text = $.Localize("#" + cat) + weapon_list[name][stat_name]
			if (originel_item["upgrade_"+stat_name] < weapon_list[name][stat_name]){
				can_evolve = false
