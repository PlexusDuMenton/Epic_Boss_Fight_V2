...
function skill_manager:create_skill_tree (hero)
  ...
  if hero.unlocked_skill == nil then hero.unlocked_skill = {} end
	print(hero:GetName())
  ...
  for k,v in pairs(hero.unlocked_skill) do
    hero.skill_tree[tonumber(k)].once_unlocked = true
  end
	skill_manager:update_skill_bonus(hero)
end

function skill_manager:update_passive_ability(hero,original_ST)
  for k,v in pairs (hero.skill_tree) do
    if hero.skill_tree[tonumber(k)].type == "stats" then
      hero.skill_tree[tonumber(k)][hero.skill_tree[tonumber(k)].stat_name] = original_ST[tonumber(k)][hero.skill_tree[tonumber(k)].stat_name]
    end
  end
end

function skill_manager:Tree_Is_outdated(hero,original_ST)
  if #hero.skill_tree ~= #original_ST then
    return true
  end

  for k,v in pairs (hero.skill_tree) do
    if hero.skill_tree[tonumber(k)].type ~= original_ST[tonumber(k)].type then
      print("type is different")
      return true
    end
    if hero.skill_tree[tonumber(k)].max_lvl~= nil and original_ST[tonumber(k)].max_lvl ~= nil and hero.skill_tree[tonumber(k)].max_lvl < original_ST[tonumber(k)].max_lvl then
      print("max_lvl is different")
      return true
    end
    if hero.skill_tree[tonumber(k)].lvl_gap ~= original_ST[tonumber(k)].lvl_gap then
      print("lvl_gap is different")
      return true
    end
    if hero.skill_tree[tonumber(k)].file_name ~= original_ST[tonumber(k)].file_name then
      print("file_name is different")
      return true
    end
    if original_ST[tonumber(k)].unlock_id ~= nil and hero.skill_tree[tonumber(k)].unlock_id then
      if #hero.skill_tree[tonumber(k)].unlock_id ~= #original_ST[tonumber(k)].unlock_id then
        return true
      end
      for j,t in pairs(hero.skill_tree[tonumber(k)].unlock_id) do
        if hero.skill_tree[tonumber(k)].unlock_id[tonumber(j)] ~= original_ST[tonumber(k)].unlock_id[tonumber(j)] then
          return true
        end
        if hero.skill_tree[tonumber(k)].stat_name ~= hero.skill_tree[tonumber(k)].stat_name then
          return true
        end
      end
    end
...
function skill_manager:load_skill_tree (hero,skill_tree,active_list)
  ...
  local O_skill_tree = hero.skill_tree
	hero.skill_tree = {}
	hero.active_list = {}
	hero.active_ability = 0
  ...
  if skill_manager:Tree_Is_outdated(hero,O_skill_tree) == true then
    Notifications:Inventory(hero:GetPlayerID(), {text="#IMCOMPATIBLE_SKILL_TREE",duration=5,color="FFAAAA"})
    skill_manager:create_skill_tree (hero)
    ...
    for k,v in pairs(active_list)do
      v.equip = false
      hero.skill_tree[v.ID].equip = false
      skill_manager:equip_skill (hero,v.ID,v.slot)
    end
  	skill_manager:skill_table (hero)
  	skill_manager:update_skill_bonus(hero)
  end
			...
			if hero.Level >= (hero.skill_tree[ID].min_lvl + hero.skill_tree[ID].lvl*hero.skill_tree[ID].lvl_gap) then
				if hero.skill_tree[ID].active == false then
					...
					for k,U_ID in pairs(hero.skill_tree[ID].unlock_id) do
						hero.skill_tree[tonumber(U_ID)].unlocked = true
            hero.unlocked_skill[tonumber(U_ID)] = true
						hero.skill_tree[tonumber(U_ID)].lvl = 0
					end
				end
...
function skill_manager:equip_skill (hero,ID,slot)
	...
	if slot == nil and hero.active_ability < 4 then
		if hero.skill_tree[ID].lvl > 0 and hero.skill_tree[ID].equip == false and hero.skill_tree[ID].type == "active" or hero.skill_tree[ID].type == "ultimate" then
			print ("equip a skill ans slot empty")
    ...
    print ("try to equip a skill")
    print(ID,hero.skill_tree[ID].name)
    if hero.active_list[slot]~= nil then
        print(slot,hero.active_list[slot].name)
    end
    print(hero.skill_tree[ID].lvl,hero.skill_tree[ID].equip,hero.skill_tree[ID].type)
		if hero.skill_tree[ID].lvl > 0 and hero.skill_tree[ID].equip == false and hero.skill_tree[ID].type == "active" or hero.skill_tree[ID].type == "ultimate" then
			print ("equip a skill and slot not empty")
			if ID >= 0 then
        print ("remove "..hero.active_list[slot].name)
...
function skill_manager:create_node (skill,hero)
	...
	skill_result.lvl = 0
  skill_result.once_unlocked = false
	if skill_result.unlocked == "true" then
    skill_result.unlocked = true
    hero.unlocked_skill[skill_result.ID] = true
  else
    skill_result.unlocked = false
  end
...
function skill_manager:Order (skill,hero)
	...
	if skill.unlock_id~= nil then
		for k,U_ID in pairs(skill.unlock_id) do
			...
			hero.skill_tree[tonumber(U_ID)].unloked_by = skill.ID
			hero.skill_tree[tonumber(U_ID)].Lvl_Position = skill.Lvl_Position + 1
		end
	end
end
