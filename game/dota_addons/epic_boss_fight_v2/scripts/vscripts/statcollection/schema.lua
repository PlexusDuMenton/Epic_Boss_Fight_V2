...
function customSchema:submitRound(winner)

    local winners = BuildRoundWinnerArray(winner)
    local game = BuildGameArray()
    local players = BuildPlayersArray()
...
function BuildRoundWinnerArray(winner)
    local winners = {}
    local current_winner_team = winner --You'll need to provide your own way of determining which team won the round
    for playerID = 0, DOTA_MAX_PLAYERS do
        if PlayerResource:IsValidPlayerID(playerID) then
            if not PlayerResource:IsBroadcaster(playerID) then
