	        ...
	        for _,wearable in pairs(wearables) do
	            if wearable:GetClassname() == "dota_item_wearable" then
	                if not wearable:GetModelName():match("tree") then
	                    local new_model_name = string.gsub(wearable:GetModelName(),"1","3")
	...
	if thisEntity:IsAlive() or not thisEntity:IsNull() then
		...
		Timers:CreateTimer(0.1,function()
			...
			if count_rock < rocks then
				...
				thisEntity:SetHealth(thisEntity:GetHealth()-thisEntity:GetMaxHealth()*0.04167)
				Timers:CreateTimer(0.1,function()
					rock:SetMaxHealth(thisEntity:GetMaxHealth()*0.0833)
...
function Spell_2()
	if thisEntity:IsAlive() or not thisEntity:IsNull() then
		...
		local rocks = 4 + math.floor(GameRules.player_difficulty*0.6) + RandomInt(0,2)

		if rocks > 7 then rocks = 7 end
	...
	if thisEntity:IsAlive() or not thisEntity:IsNull() then
		...
		Timers:CreateTimer(0.1,function()
			...
			if count_rock < rocks then
				...
				local rock = boss_manager:Spawn_Boss("rockling_1",thisEntity:GetAbsOrigin(),10)
				LinkLuaModifier( "immunity", "modifiers/immunity.lua", LUA_MODIFIER_MOTION_NONE )
				thisEntity:SetHealth(thisEntity:GetHealth()-thisEntity:GetMaxHealth()*0.05)
				rock:AddNewModifier(rock, nil, "immunity", {duration = 1})
				rock.free = false
				rock.parent = thisEntity
				Timers:CreateTimer(0.1,function()
					rock:SetMaxHealth(thisEntity:GetMaxHealth()*0.1)
					rock:SetHealth(rock:GetMaxHealth())
				end)
...
function spawn_next_form()
	...
	boss_manager:Spawn_Boss("boss_5_4",thisEntity:GetAbsOrigin(),0)
end
