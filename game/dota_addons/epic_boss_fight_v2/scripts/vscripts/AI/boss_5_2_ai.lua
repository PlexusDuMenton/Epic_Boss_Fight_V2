	        ...
	        for _,wearable in pairs(wearables) do
	            if wearable:GetClassname() == "dota_item_wearable" then
	                if not wearable:GetModelName():match("tree") then
	                    local new_model_name = string.gsub(wearable:GetModelName(),"1","2")
...
function Spell_2()
	if thisEntity:IsAlive() or not thisEntity:IsNull() then
		...
		local rocks = 2 + math.floor(GameRules.player_difficulty*0.6) + RandomInt(0,2)

		if rocks > 5 then rocks = 5 end
...
function spawn_next_form()
	...
	boss_manager:Spawn_Boss("boss_5_3",thisEntity:GetAbsOrigin(),0)
end
